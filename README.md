...
- **Panning** — per-string stereo modes (Center, Equal Spread 1/2, Split 1/2, Zig-Zag 1/2, Narrow, Pairs) driving `INST:STRING(A) PAN 1–6` (CC# 71–76 by default). Hovering a mode previews it live in the Current Tuning readout; clicking sends the pan CCs.
- **Current Tuning readout** — one at-a-glance panel (parked in the left **sidebar** on wide screens, inline otherwise) that combines pitch and pan for all six strings: a top **pan strip** of colored, numbered dots placed L↔R, linked by a fixed per-string color to the **pitch neck** below (each string's note slides flat↔sharp from standard, with its offset and a matching color swatch). Everything updates live — and animates — from the current tuning + pedals + pan.
