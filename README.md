...
- **Panning** — per-string stereo modes (Center, Equal Spread 1/2, Split 1/2, Zig-Zag 1/2, Narrow, Pairs) driving `INST:STRING(A) PAN 1–6` (CC# 71–76 by default), with a stereo-field display showing where each string sits L↔R and a live hover preview.
- **Overview (Linked Twin Views)** — a read-only status panel that shows all six strings at once: a top **pan field** (colored, numbered dots placed L↔R by pan) linked by a fixed per-string color to a bottom **pitch neck** (each string's note placed flat↔sharp from standard, with a matching color swatch). Both strips update live from the current tuning + pedals + pan.
