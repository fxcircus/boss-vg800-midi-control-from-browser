...
The **GUITAR card** (under Bends) is the manual side of the same control: Electric/Acoustic tabs, a chip per model and a line-art portrait of the current one. Picking a chip sends the same two-message sequence immediately — even with *Guitar model* auto-select **Off** (a click is explicit intent; Auto only governs what happens when a tuning is applied). With Auto **on**, the next tuning you apply takes the model back over. The card lives in Classic only — in Panning those CCs mean pan and 12-string.
...
- **Guitar picker** *(Classic mode)* — a **GUITAR card** under Bends with Electric/Acoustic tabs and all **21 VG-800 models** (CLA-ST → FRETLESS, MA28 → SITAR) as chips, each drawn as a recognizable line-art portrait in the preview pane. A chip click switches the hardware model instantly via CC# 70 + 77/78 and works even when auto-select is Off; tuning cards keep auto-picking their own model while Auto is on.
