...
- **Tuning cards** — click to retune all six strings. Families: **Common** (Standard, DADGAD, Nashville, Baritone, Bass VI, Hendrix), **Open Majors**, **Drop**, and an **Artists** set (Fripp NST, Gambale, Page's Led Kashmir and Led Rain, Sonic Youth, Nick Drake, Keith Richards, American Football, SoundG Wave and SoundG Sun, Young Cinnamon, Joni Cab, Joni Sides, Radio Pyramid, Radio Everything). Each card shows its note names and per-string ± offset. **See [TUNINGS.md](TUNINGS.md) for the complete list** (every tuning with its notes and offsets).
...
- **Fretboard visualizer** — the fretboard button (header, left of the theme pedal) opens the [guitar-fretboard-visualizer](https://github.com/fxcircus/guitar-fretboard-visualizer) in a modal: the current tuning drawn on a playable neck with its scale, every chord under a straight bar, and a physically-modelled steel tone. It follows every tuning card you click (steels show the full 10-string neck; customs, modes and transposes show the actual pitches), and picking a tuning in its dropdown activates the matching card and retunes the hardware. Inherits the active theme; ships in `fretboard/` as a prebuilt module from that repo (`npm run sync:vg800` there).
